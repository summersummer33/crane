...
class YOLOv5Detector:
    def __init__(self,
                 ...
                 max_det=1000,
                 line_thickness=3,
                 view_img=True):  # 新增一个参数来控制是否显示图像窗口
        """
        YOLOv5检测器类，用于封装模型的加载和推理。
        模型只在初始化时加载一次，解决重复加载导致的性能问题。
        此版本内置了打印信息和图像显示功能。
        """
        ...
        self.view_img = view_img  # 保存显示图像的选项

        # 如果需要显示图像，则提前设置好窗口
        self.window_name = "YOLOv5 Detection"
        if self.view_img:
            # 兼容不同操作系统，避免Linux下调整窗口大小的问题
    ...
    def detect(self, im0):
        ...
        内置功能：
        1. 在控制台打印检测信息。
        2. 在一个OpenCV窗口中实时显示带标注的图像。
        
        :param im0: 原始图像 (OpenCV格式, BGR)
        :return: det: 检测结果tensor，格式为 [x1, y1, x2, y2, conf, cls]
        """
        # 预处理图像
        ...
        if len(det):
            # 将检测框的坐标从模型输入尺寸(imgsz)缩放回原始图像尺寸(im0)
            det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], im0.shape).round()

            # 准备打印信息
            for c in det[:, 5].unique():
                n = (det[:, 5] == c).sum()  # 某个类别的检测数量
            ...
            if self.view_img:
                ...
                cv2.waitKey(1)  # 必须有 waitKey 来刷新窗口并处理事件

        else: # 如果没有检测到任何物体
            if self.view_img:
                # 即使没有检测到，也显示原始图像，以保持视频流的连贯性
